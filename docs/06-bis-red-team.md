...
Chaque étape se fait **au rythme de la furtivité**, pas de l'efficacité brute. Là où un pentester enchaîne, un opérateur red team **attend, observe, dose** - parce qu'il sait qu'un SOC corrèle les événements (ta Partie 6, [module 60](06-purple-team.md#module-60-le-pivot-blue-team-detection-remediation-transverses), mais vue depuis l'autre côté).
...
- On mappe chaque technique à **ce que la Blue Team devrait voir** (data sources ATT&CK) → c'est déjà la grille de restitution du [module 67](#module-67-le-livrable-rapport-timeline-restitution-blue-team).
...
2. Avec **Atomic Red Team**, exécute 3 tests atomiques (ex. Kerberoasting, DCSync, création de service) et vérifie que **tes** règles de détection (P6 [M60](06-purple-team.md#module-60-le-pivot-blue-team-detection-remediation-transverses)) s'allument.
...
- **Détection comportementale** : chaîne parent/enfant suspecte (`winword.exe`→`powershell.exe`→réseau), LOLBins hors contexte, injections, création de services distants (P6 [M57](06-purple-team.md#module-57-mouvement-lateral)).
...
- **Threat hunting** : chasser proactivement les LOLBins, les beacons ([M64](#module-64-infrastructure-offensive-command-and-control-c2)), les anomalies - sans attendre l'alerte.
...
## 65.5 Exercice n°55
1. Déploie **Sysmon** (config de référence) sur `corp.lab.local`, remonte vers ton SIEM (P6 [M60](06-purple-team.md#module-60-le-pivot-blue-team-detection-remediation-transverses)).
...
- **Phishing / spear-phishing** : mail ciblé menant à un vol d'identifiants (fausse page) ou à l'exécution d'un leurre (pièce jointe/lien → implant, [M64](#module-64-infrastructure-offensive-command-and-control-c2)).
...
Cette table **est** le pont vers ton [module 60](06-purple-team.md#module-60-le-pivot-blue-team-detection-remediation-transverses) (détection) et 63 (heatmap ATT&CK). Elle transforme une intrusion en **feuille de route défensive chiffrée** : « votre temps de détection moyen est de X, vous avez raté N étapes sur M, priorité à ceci ».

## 67.4 Les métriques qui parlent au management

- **MTTD / MTTR** : temps moyen de détection / de réponse (les métriques reines du SOC).
- **Taux de détection** : % d'étapes ATT&CK détectées (la heatmap, [M63](#module-63-emulation-dadversaire-threat-intelligence)).
- **Dwell time** simulé : combien de temps l'attaquant serait resté invisible.
- **Taux de signalement** humain (phishing, [M66](#module-66-la-couche-humaine-acces-initial-ingenierie-sociale)).
...
- **Nouvelles détections** : chaque « raté » de la timeline devient une **règle** (P6 [M60](06-purple-team.md#module-60-le-pivot-blue-team-detection-remediation-transverses)) que la blue écrit et **teste** (Atomic Red Team, [M63](#module-63-emulation-dadversaire-threat-intelligence)).
- **Re-test** : plus tard, on rejoue les mêmes TTP → les trous sont-ils bouchés ? C'est la mesure du progrès.
- **Postmortem blameless** (rappel P4 [M35](04-exploitation-sre.md#module-35-pratiques-sre-appliquees-a-un-parc-windows)) : on cherche la faille systémique, jamais le coupable.
...
- [ ] Plan d'opération (objectif/crown jewel, *assume breach*, ROE) - [module 62](#module-62-red-team-vs-pentest-vs-purple-cadre-cycle).
- [ ] Plan d'émulation ATT&CK d'un adversaire choisi + heatmap de couverture - [module 63](#module-63-emulation-dadversaire-threat-intelligence).
- [ ] C2 opéré dans le lab, callback caractérisé et **détecté** côté blue - [module 64](#module-64-infrastructure-offensive-command-and-control-c2).
- [ ] Sysmon + SIEM instrumentés ; détection comportementale et d'aveuglement - [module 65](#module-65-opsec-evasion-concepts-et-detection).
- [ ] Accès initial social simulé sur comptes de test + défenses (DMARC, FIDO2, ASR) - [module 66](#module-66-la-couche-humaine-acces-initial-ingenierie-sociale).
